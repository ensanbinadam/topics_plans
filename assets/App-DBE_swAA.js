...
`,Ee=async t=>{const o=document.getElementById("printable-area-content"),{meta:r,title:n}=t,{useEasternDigits:a}=r;if(!o){alert("حدث خطأ: لا يمكن العثور على المحتوى للتصدير.");return}const m=`
      <div class="mb-2" id="main-header">
        <div class="print-header-row">
          ...
          <div class="print-col-center">
             <div id="header-title" class="print-text-lg">${k(n,a)}</div>
             <div id="header-admin" class="print-text-base">${k(`إدارة التعليم: ${r.administration}`,a)}</div>
             <div id="header-school" class="print-text-base">${k(`المدرسة: ${r.school}`,a)}</div>
          </div>
          
          <!-- Left Column -->
          <div class="print-col-left">
             <div id="header-year" class="print-text-sm">${k(`العام: ${r.academicYear}`,a)}</div>
             <div id="header-term" class="print-text-sm">${k(`الفصل: ${r.term}`,a)}</div>
             <div id="header-subject" class="print-text-sm">${k(`المادة: ${r.subject}`,a)}</div>
             <div id="header-grade" class="print-text-sm">${k(`الصف: ${r.grade}`,a)}</div>
          </div>
        </div>
  ...
  `,h=o.cloneNode(!0);h.querySelectorAll("textarea[data-row-index]").forEach($=>{var C;const W=$,F=document.createElement("div");F.innerHTML=W.value.replace(/\n/g,"<br />");const b=W.closest("tr"),f=b&&b.classList.contains("print:hidden");F.className=`p-2 w-full h-full ${W.disabled?"text-center":""}`,(W.disabled||f)&&F.classList.add("bg-gray-200"),(C=W.parentNode)==null||C.replaceChild(F,W)});const c=Array.from(document.querySelectorAll("style")).map($=>$.innerText).join(`
`),l=Array.from(document.querySelectorAll("link[rel='stylesheet'], link[rel='preconnect']")).map($=>$.outerHTML).join(`
`),u=`
...
`,j=new Blob([E.trim()],{type:"text/html;charset=utf-8;"}),L=`school_plan_${t.startDate||"new"}.html`;Y(j,L)},te=t=>{var d,h;const o=document.getElementById("printable-area-content"),r=(d=document.getElementById("printable-header-content"))==null?void 0:d.innerHTML,n=(h=document.getElementById("printable-footer-content"))==null?void 0:h.innerHTML;if(!o||!r||!n){alert("حدث خطأ أثناء محاولة الطباعة.");return}const a=o.cloneNode(!0);a.querySelectorAll("textarea[data-row-index]").forEach(c=>{var $;const l=c,u=document.createElement("div");u.style.whiteSpace="pre-wrap",u.style.wordWrap="break-word",u.textContent=l.value;const E=l.disabled,j=l.closest("tr"),L=j&&j.classList.contains("print:hidden");u.className=`p-2 w-full h-full ${E||L?"text-center":""}`,($=l.parentNode)==null||$.replaceChild(u,l)});const m=Array.from(document.querySelectorAll("style")).map(c=>c.innerText).join(`
...
`),x=window.open("","_blank");if(!x){alert("فشل فتح نافذة الطباعة.");return}x.document.write(`
    ...
    </html>`),x.document.close(),setTimeout(()=>{try{x.focus(),x.print(),x.close()}catch(c){console.error(c),x.close()}},1200)},We=async t=>{var a,m;const o=document.getElementById("printable-area-content"),r=(a=document.getElementById("printable-header-content"))==null?void 0:a.innerHTML,n=(m=document.getElementById("printable-footer-content"))==null?void 0:m.innerHTML;if(!o){alert("لا يوجد محتوى للنسخ.");return}try{const y=o.cloneNode(!0);y.querySelectorAll("textarea").forEach(u=>{var L;const E=u,j=document.createElement("div");j.innerHTML=E.value.replace(/\n/g,"<br>"),j.style.whiteSpace="pre-wrap",(L=E.parentNode)==null||L.replaceChild(j,E)}),y.querySelectorAll(".print\\:hidden").forEach(u=>u.remove());const x=`
      <html dir="rtl">
        <head>
          <meta charset="utf-8">
          <style>
