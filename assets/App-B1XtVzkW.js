...
`,$e=async t=>{const o=document.getElementById("printable-area-content"),{meta:r,title:n}=t,{useEasternDigits:i}=r;if(!o){alert("حدث خطأ: لا يمكن العثور على المحتوى للتصدير.");return}const f=`
  ...
  `,p=o.cloneNode(!0);p.querySelectorAll("textarea[data-row-index]").forEach(k=>{var T;const W=k,L=document.createElement("div");L.innerHTML=W.value.replace(/\n/g,"<br />");const m=W.closest("tr"),h=m&&m.classList.contains("print:hidden");L.className=`p-2 w-full h-full ${W.disabled?"text-center":""}`,W.classList.contains("text-red-600")&&L.classList.add("text-red-600"),(W.disabled||h)&&L.classList.add("bg-gray-200"),(T=W.parentNode)==null||T.replaceChild(L,W)});const l=Array.from(document.querySelectorAll("style")).map(k=>k.innerText).join(`
