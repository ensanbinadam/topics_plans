...
`);if(!o){alert("لا توجد موضوعات مكتوبة لتصديرها.");return}const r=new Blob(["\uFEFF"+o],{type:"text/plain;charset=utf-8;"});Y(r,"topics.txt")},We=t=>{if(!t.days.length){alert("لا يوجد جدول لتصديره.");return}const{days:o,meta:r}=t,n=r.hiddenWeeks||[],i=r.useEasternDigits,j=o.reduce((k,w,N)=>((w.gregorianDate.getDay()===0||N===0)&&k.push([]),k[k.length-1].push(w),k),[]).map((k,w)=>({week:k,originalIndex:w})).filter(k=>!n.includes(k.originalIndex));let b=0,l="";const h=4;for(let k=0;k<j.length;k+=h){l+='<tr style="height: 140px;">';for(let w=0;w<h;w++){const N=k+w,T=j[N];if(!T){l+='<td style="border: 1px solid #000; padding: 5px; vertical-align: top;"></td>';continue}b++;const B=b,p=T.week,g=p.filter(H=>{const z=H.gregorianDate.getDay();return z!==5&&z!==6}),W=g.length>0?g[0].gregorianDate:p[0].gregorianDate,S=g.length>0?g[g.length-1].gregorianDate:p[p.length-1].gregorianDate,$=`(${u(`${J(W)} - ${J(S)}`,i)}هـ | ${u(`${_(W)} - ${_(S)}`,i)}م)`,C=p.filter(H=>{const z=H.gregorianDate.getDay(),L=z===5||z===6;return!(r.hideWeekends&&L)&&H.topic&&H.topic.trim()!==""&&H.topic.trim()!=="--- عطلة نهاية الأسبوع ---"&&H.topic.trim()!=="--- إجازة رسمية ---"});let A="";C.length>0?A='<ol style="margin: 0; padding-right: 1.2em; font-size: 10pt;">'+C.map(H=>`<li style="margin-bottom: 2px;">${H.isSpecial?`<strong>${H.topic}</strong>`:H.topic}</li>`).join("")+"</ol>":A='<p style="text-align: center; color: #777; font-size: 10pt; margin-top: 20px;">-</p>',l+=`
                <td style="border: 1px solid #000; padding: 5px; vertical-align: top; width: 25%;">
                    <div style="border-bottom: 1px solid #ccc; margin-bottom: 4px; padding-bottom: 2px; text-align: center;">
                        <span style="font-weight: bold; font-size: 12pt; color: #2b6cb0;">الأسبوع ${u(B,i)}</span>
                        <br/>
                        <span style="font-size: 8pt; color: #555;">${$}</span>
                    </div>
                    ${A}
                </td>
            `}l+="</tr>"}const d=`
        <table style="width: 100%; border: none; margin-bottom: 10px; border-bottom: 2px solid #000;">
            <tr>
                <td style="width: 20%; text-align: right; vertical-align: middle; border: none;">
                    ${r.logo?`<img src="${r.logo}" width="100" height="80" alt="Logo" />`:""}
                </td>
                <td style="width: 60%; text-align: center; vertical-align: middle; border: none;">
                    <h2 style="font-size: 16pt; margin: 0; padding: 0;">${u(t.title,i)}</h2>
                    <p style="font-size: 12pt; margin: 3px 0;">${u(`إدارة التعليم: ${r.administration}`,i)}</p>
                    <p style="font-size: 12pt; margin: 3px 0;">${u(`المدرسة: ${r.school}`,i)}</p>
                </td>
                <td style="width: 20%; text-align: center; vertical-align: middle; border: none;">
                    <p style="font-size: 10pt; margin: 2px 0;">${u(`العام: ${r.academicYear}`,i)}</p>
                    <p style="font-size: 10pt; margin: 2px 0;">${u(`الفصل: ${r.term}`,i)}</p>
                    <p style="font-size: 10pt; margin: 2px 0;">${u(`المادة: ${r.subject}`,i)}</p>
                    <p style="font-size: 10pt; margin: 2px 0;">${u(`الصف: ${r.grade}`,i)}</p>
                </td>
            </tr>
        </table>
    `,c=`
        <div style="margin-top: 20px; text-align: center; font-weight: bold; font-size: 12pt; display: flex; justify-content: center; gap: 50px;">
    ...
    `,y=`
        ...
        <head>
            ...
            <style>
                ...
                /* Typography adjustments for Word */
                p { margin: 0; }
    ...
    `,I=new Blob(["\uFEFF",y],{type:"application/msword"});Y(I,`plan_grid_${t.startDate}.doc`)},Y=(t,o)=>{const r=URL.createObjectURL(t),n=document.createElement("a");n.href=r,n.download=o,document.body.appendChild(n),n.click(),document.body.removeChild(n),URL.revokeObjectURL(r)},ne=()=>`
...
`,k=new Blob([I.trim()],{type:"text/html;charset=utf-8;"}),w=`school_plan_${t.startDate||"new"}.html`;Y(k,w)},te=t=>{var l,h;const o=document.getElementById("printable-area-content"),r=(l=document.getElementById("printable-header-content"))==null?void 0:l.innerHTML,n=(h=document.getElementById("printable-footer-content"))==null?void 0:h.innerHTML;if(!o||!r||!n){alert("حدث خطأ أثناء محاولة الطباعة.");return}const i=o.cloneNode(!0);i.querySelectorAll("textarea[data-row-index]").forEach(d=>{var N;const c=d,y=document.createElement("div");y.style.whiteSpace="pre-wrap",y.style.wordWrap="break-word",y.textContent=c.value;const I=c.disabled,k=c.closest("tr"),w=k&&k.classList.contains("print:hidden");y.className=`p-2 w-full h-full ${I||w?"text-center":""}`,(N=c.parentNode)==null||N.replaceChild(y,c)});const f=Array.from(document.querySelectorAll("style")).map(d=>d.innerText).join(`
